...
prec_grade <- res %>% 
        select(act_group_stn,act_id,actgrp_char,sub_char,LASAR_ID,CharIDText,sample_type,Result.Value,DateTime,Characteristic.Name,result_id,LOQ) %>%
        left_join(QC_calc_M, by = c('act_group_stn','CharIDText')) %>% 
        group_by(act_group_stn,CharIDText) %>% 
        mutate(n = n(),
               Dup = ifelse(n() > 1, 1, 0)) %>%
        ungroup() %>% 
        filter(Dup == 1) %>% # pulls out primary and dups pairs 
        select(act_group_stn,act_id,LASAR_ID,DateTime,Characteristic.Name,CharIDText,actgrp_char,sub_char,sample_type,Result.Value,result_id,QCcalc,DQLA,DQLB,LOQ) %>%
        group_by(act_group_stn,CharIDText) %>%
        ...
        mutate(prec_val = case_when(QCcalc == "LogDiff" ~
                                    abs(log10(Result.Value) - log10(lag(Result.Value, default = first(Result.Value)))),
                                    QCcalc == "AbsDiff" ~ abs(Result.Value - lag(Result.Value, default = first(Result.Value))),  # using abs here may mess up the charts and statistics.
                                    QCcalc == "RPD" ~ 
                                    abs((Result.Value - lag(Result.Value, default = first(Result.Value)))/mean(Result.Value))),
        ...
        select(act_group_stn,CharIDText,result_id,QCcalc,prec_val,DQLA,DQLB,prec_DQL)  # 

# add prec grade to result, 
res_prec_grade <- res %>% 
  left_join(prec_grade, by = c('act_group_stn','result_id','CharIDText')) #%>% # This seems to assign DQL to only dup rather than FP and FD
...
Prelim_DQL_AG_char <- res_prec_grade %>% 
                      select(act_group_stn,act_id,LASAR_ID,DateTime,subid,act_type,sample_type,TypeIDText,TypeShortName,Date4Id,Date4group,act_id,
                      act_group,CharIDText,result_id,sub_char,actgrp_char,Result.Value,DQLA,DQLB,prec_val,prec_DQL) %>%
...
if(nrow(Prelim_DQL_Mixed)>0){
  ...
  left_join(prec_grade, by = c('act_group_stn','result_id','CharIDText')) %>% #changed input of Prelim_DQL_Mixed
  group_by(act_group_stn,CharIDText) %>%
  arrange(desc(sample_type), .by_group = TRUE) %>%
  #mutate(prelim_dql = ifelse(sample_type == 'dup' & !is.na(prec_DQL),prec_DQL,prelim_dql)) %>%
  select(act_group_stn,LASAR_ID,CharIDText,act_id,act_group,result_id,actgrp_char,sample_type,prec_DQL,prelim_dql, DateTime) %>%
  arrange(actgrp_char) %>%
...
{sub_char_precen <- Prelim_DQL_All %>%
                group_by(CharIDText) %>% 
                summarise(percen_5th = quantile(Result.Value, probs = .05),
...
anom <- Prelim_DQL_All %>% 
  #rename(LOQ = 'Limit of Quantitation') %>%
        ...
        select(LASAR_ID,DateTime,CharIDText,subid,TypeIDText,Result.Value,act_id,act_group,result_id,AnomCritID,charid,LOQ,RealRngL,RealRngU,
        ...
        mutate(expected_range = ifelse(Result.Value > RealRngU | Result.Value < RealRngL,"OutOfRange",NA),
               LOQ_anom = ifelse(Result.Value < LOQ-(LOQ*0.1),"BelowLOQ", NA),
               ambient_01 = ifelse(Result.Value < amb01L,"Lowest1%AmbientValues",NA),
               ambient_05 = ifelse(Result.Value < amb05L,"Lowest5%AmbientValues", NA),
               ambient_95 = ifelse(Result.Value > amb95U,"Highest95%AmbientValues", NA),
               ambient_99 = ifelse(Result.Value > amb99U,"Highest99%AmbientValues", NA),
               WQS = ifelse(Result.Value > stdU | Result.Value < stdL, "ViolatesWQStandard",NA),
               WQS_LP = ifelse(Result.Value > stdUlp | Result.Value < stdLlp, "ViolatesLessProtectiveWQStandard",NA),
               sub_5 = ifelse(Result.Value < percen_5th,"Lowest5%SubmValues",NA),
               sub_10 = ifelse(Result.Value < percen_10th,"Lowest10%SubmValues",NA),
               sub_90 = ifelse(Result.Value > percen_90th,"Highest90%SubmValues",NA),
               sub_95 = ifelse(Result.Value > percen_95th,"Highest95%SubmValues",NA),
               station_5 = ifelse(Result.Value < S_percen_5th,"Lowest5%SubmStnValues",NA),
               station_95 = ifelse(Result.Value > S_percen_95th,"Highest95%SubmStnValues",NA)) %>%
        ...
        gather(key = "Anom_type", value = "Anom",-c(LASAR_ID,DateTime,CharIDText,subid,TypeIDText,
                                                    Result.Value,act_id,act_group,result_id,AnomCritID,charid,LOQ,RealRngL,RealRngU,
