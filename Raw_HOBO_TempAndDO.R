...
###############################################################################
# Format HOBO temperature and DO logger data                                  #
###############################################################################

## Create lists of the temp and datetime columns based on the column names in your data
DO_columns <- names(raw_data)[grep("^DO.conc", names(raw_data))]
temp_columns <- names(raw_data)[grep("^Temp", names(raw_data))] 
datetime_columns <- names(raw_data)[grep("^Date", names(raw_data))] 

## Prep the dataset by combining columns, formatting dates, and dropping unnecessary columns
data <- raw_data %>%
  rename(DateTime1 = all_of(datetime_columns[1]), # rename the singled out columns
         Temp1 = all_of(temp_columns[1]),
         DO1 = all_of(DO_columns[1])) %>%
  ...
  reduce(DO_columns[-1], .init = ., # Iterate through all the DO columns and combine them into DO1
         ~ mutate(.x, DO1 = if_else(is.na(DO1) & !is.na(.data[[.y]]), .data[[.y]], DO1))) %>%
  select(DateTime1, Temp1, DO1, equipID) %>% # Drop everything besides what's listed in the select statement
...
results <- data %>%
  ...
  mutate("Activity Start Date" = as.Date(DateTime1), # add necessary columns to fill out the Results tab
         ...
         "Result Unit" = if_else(Char_Name == "Temp", "deg C", "mg/L"),
         Char_Name = if_else(Char_Name == "Temp1", "Temperature, water", 
                             "Dissolved Oxygen (DO)"),
         "Result Status ID" = "Validated") %>%
  select(-DateTime1, -is_dst) %>% # Drop columns not on the Results tab
  relocate(c(equipID, Characteristic.Name, Result), .before = "Result Unit") %>%
  rename("Monitoring Location ID" = MLocID, "Equipment ID  #" = equipID,
         "Characteristic Name" = Characteristic.Name, "Result Value" = Result)
