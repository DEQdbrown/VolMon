...
# Variables
# Update path with the file you're using each time you run the script

project = 'MS4 Annual Reporting'
type = "file"
path = "//Deqhq1/stormwater/Muni Stormwater Program/Municipal - Phase I/Electronic Data Delivery/2024/Working Copies/Salem/WorkingCopy_CoS_Continuous_QC_2023-24_1.xlsx"
...
alternate_project_ID <- NA_character_
...
Results_import <- readxl::read_xlsx(path, sheet = "Results", col_types = results_col_types)
...
#If you receive a warning due to datetime not parsing, check to make sure that the date in the Activity Start Time column of the import file isn't 2/29/1900.
#If it is, then delete the date from the time column, save and rerun the script.
...
write.csv(deployments, paste0(tools::file_path_sans_ext(path),"-deployments.csv"), row.names = FALSE)
...
AQWMS_sum_stat <- sumstat_long %>%
  ...
  mutate(RsltTimeBasis = ifelse(StatisticalBasis == "7DMADMin" |
         ...
         ActivityType = "FMC",
         Result.Analytical.Method.ID = case_when(charID == "Conductivity"~ "120.1",
                                                 charID == "Dissolved oxygen (DO)" |
                                                   charID == "Dissolved oxygen saturation"~ "NFM 6.2.1-LUM",
                                                 charID == "pH"~"150.1",
                                                 charID == "Temperature, water"~ "170.1",
                                                 charID == "Turbidity"~ "180.1",
                                                 charID == 'Total dissolved solids' ~ '160.1',
                                                 TRUE ~ 'ERROR'),
...
openxlsx::write.xlsx(AQWMS_sum_stat, paste0(tools::file_path_sans_ext(path),"-statsum.xlsx"))
...
if(nrow(cont_pH_AWQMS) > 0){
  # Export to same place as the originial file
  openxlsx::write.xlsx(cont_pH_AWQMS, paste0(tools::file_path_sans_ext(path),"-cont_pH.xlsx"))
}
...
ggplot2::ggsave(paste0(tools::file_path_sans_ext(path),"-Graph.png"), plot = graph)
