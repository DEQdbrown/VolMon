...
### Prepare the workspace by assigning variables, setting working directory and connecting to SQL
sub_id <- 305 # change this each time
...
load(str_glue("Step1_{sub_id}.RData"))


############ Trouble Shooting Section - Run when needed #########################

### If you need to stop part way through the script, run this line
save.image(file = str_glue("Step2_{sub_id}.RData"))

### If you stopped part way and are starting back up, run this line
load(str_glue("Step2_{sub_id}.RData"))

#### If connection to database fails (___ is not an open RODBC channel), close the connection and then reopen. KM 10.2023 ####
odbcClose(VM2.sql)
...
### confirm load number matched anom
QC_Anom <- sqlQuery(VM2.sql, qryAnom)
...
### confirm load number matched gd4r
QC_gd4r <- sqlQuery(VM2.sql, qrygd4r) 
