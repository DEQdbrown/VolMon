...
setwd("//deqlab1/Vol_Data/Coos/SWCD/2024/e.Coli/R") ##### change this each time!!!!!!!!!!!!!!
data_path <- "//deqlab1/Vol_Data/Coos/SWCD/2024/e.Coli/WorkingCopy_CoosSWCD_NF_COQ_SIA_2024_Grab_Ecoli.xlsx" ##Change this also

VM2.sql <- odbcConnect("VolMon2")
load("Step1.RData") ### this is dependent on the submission, should update to look like this "step1_308.RData" 

############################################################################################################################################
######################### Don't forget to update the folder and file name at the bottom of the script!!##############################################
...
db_res <- sqlQuery(VM2.sql, qryRest) %>% 
          ...
          mutate(comment = ifelse(!is.na(Org_RsltComment),paste(Org_RsltComment,DEQ_RsltComment, sep = ":"),
                 ...
                 result = ifelse(!is.na(RsltQual),paste(RsltQual,Result, sep = ""),
                                 Result),
                 LOQUnit = UnitIdText,
                 RQM = case_when(ORDEQ_DQL == 'B' ~ 'FQC',
                                 ORDEQ_DQL == 'E'|ORDEQ_DQL == 'C' ~ 'SUS',
                                 TRUE ~ "")) %>% 
  select(ActivityTypeId,MLocID,StartDate,StartTime,StartDateTimeZoneID,
         ...
         AnEndDate,AnEndTime,AnalyticalEndTimeZoneID,LabCommentCode,LOQ,LOQUnit,BiasValue,
         PrecisionValue,StatisticalBasisID,RsltTimeBasisID,ORDEQ_DQL,comment)

# write file for upload
write.csv(db_res,"//deqlab1/Vol_Data/Powder/2022_LAM/R/AWQMS_Powder22VolWQGrabDataSub.csv",na = "",row.names = FALSE)
