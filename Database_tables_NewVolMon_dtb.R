...
DB_tables <- function(final_DQL) {
  ...
  final_DQL <- final_DQL %>%
    rename("SampleDepth" = "Bottom.Depth.Height", "SampleDepthUnit" = "Bottom.Depth.Height.Unit", "Result" = "Result.Value", "RsltQual" = "Result.Qualifier","Res_comment" = "Result.Comment") %>%
    mutate(DEQ_Comment = NA_character_,
  ...
  act <- final_DQL %>% 
    ...
    mutate(SmplDepthUnitID = ifelse(SampleDepthUnit %like% 'meters', 146,
           ...
           StartDateTimeZoneID = ifelse(dst(DateTime) == T,22, 23), # add mountain time
           EndDateTime = NA,
