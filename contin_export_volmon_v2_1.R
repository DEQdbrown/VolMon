...
contin_export_volmon_v2 <- function (file, org, projects, mloc, deployment, results, prepost, 
                                     audits, sumstats = NULL, equipment = NULL) 
{
     ...
     projects <- projects[, cols_projects_volmon()]
     mloc <- mloc[, mloc_col_names_volmon()]
     equipment <- equipment[, odeqcdr::cols_equipment()]
     deployment <- deployment[, cols_deploy_volmon_export()]
     ...
     audits <- audits[, cols_audit_volmon()]
     if (is.null(sumstats)) {
     ...
     for (sheet_name in names(xlsx_list[2:length(xlsx_list)])) {
          openxlsx::setColWidths(wb, sheet = sheet_name, cols = c(1:ncol(xlsx_list[[sheet_name]])), 
                                 widths = "auto")
          openxlsx::addStyle(wb, sheet = sheet_name, rows = c(1:nrow(xlsx_list[[sheet_name]]) + 
                                                                   1), cols = c(1:ncol(xlsx_list[[sheet_name]])), stack = TRUE, 
                             gridExpand = TRUE, style = openxlsx::createStyle(valign = "top", 
     ...
     openxlsx::addStyle(wb, sheet = "Deployment", style = dt_format, 
                        rows = 2:(nrow(deployment) + 1), cols = c(3:4), stack = TRUE, 
                        gridExpand = TRUE)
     openxlsx::addStyle(wb, sheet = "Results", style = date_format, 
                        rows = 2:(nrow(results) + 1), cols = 2, stack = TRUE, 
                        gridExpand = TRUE)
     openxlsx::addStyle(wb, sheet = "Results", style = time_format, 
                        rows = 2:(nrow(results) + 1), cols = 3, stack = TRUE, 
                        gridExpand = TRUE)
     openxlsx::addStyle(wb, sheet = "Audit_Data", style = date_format, 
                        rows = 2:(nrow(audits) + 1), cols = c(5, 7), stack = TRUE, 
                        gridExpand = TRUE)
     openxlsx::addStyle(wb, sheet = "Audit_Data", style = time_format, 
                        rows = 2:(nrow(audits) + 1), cols = c(6, 8), stack = TRUE, 
                        gridExpand = TRUE)
     openxlsx::setColWidths(wb, sheet = "Organization Details", 
                            cols = c(2:3), widths = c(20, 100))
     openxlsx::addStyle(wb, sheet = "Organization Details", rows = c(6:19), 
                        cols = c(2:3), stack = TRUE, gridExpand = TRUE, style = openxlsx::createStyle(wrapText = TRUE, 
                                                                                                      valign = "top", fontName = "Arial", fontSize = 9))
     openxlsx::addStyle(wb, sheet = "Organization Details", rows = c(6:19), 
                        cols = 2, stack = TRUE, style = openxlsx::createStyle(textDecoration = "bold", 
